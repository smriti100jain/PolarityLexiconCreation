...
flags.DEFINE_bool('word2vec_initialize', False, 'word2vec intitialize')
...
flags.DEFINE_string('word2vecembeddingpath', '/tmp/IMDB','Directory path to preprocessed text dataset.')
...
class VatxtModel(object):
  ...
  def __init__(self, cl_logits_input_dim=None):
    ...
    self.layers['embedding'] = layers_lib.Embedding(
        ...
        self.vocab_freqs, FLAGS.keep_prob_emb,FLAGS.word2vec_initialize,FLAGS.word2vecembeddingpath)
  ...
  def pretrained_variables_word2vec(self):
    return (self.layers['embedding'].trainable_weights)
  ...
  def eval_graph(self, dataset='test'):
    ...
    embedded = self.layers['embedding'](inputs.tokens)
    embdings = self.layers['embedding'].trainable_weights
    ...
    return eval_ops, var_restore_dict, embdings

  def eval_graph_embedding(self, dataset='test'):
    """Constructs classifier evaluation graph.
    ...
    """
    inputs = _inputs(dataset, pretrain=False)
    embedding_ok = self.layers['embedding']
